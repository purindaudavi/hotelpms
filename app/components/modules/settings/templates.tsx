...
function TemplateBuilder({ initial, initialCategory, defaults, saving, onCancel, onSave }: {
  ...
  return <main className="p-4"><header className="mb-4 flex items-center justify-between border-b border-line pb-4"><button onClick={onCancel} className="inline-flex items-center gap-2 font-semibold"><ArrowLeft className="h-4 w-4"/>Back to templates</button><h1 className="text-xl font-semibold">{initial ? "Edit email template" : "Create email template"}</h1><span/></header><div className="grid min-h-[760px] gap-3 xl:grid-cols-[280px_1fr_1fr]">
    ...
    <section className="flex min-h-0 flex-col rounded-lg border border-line"><div className="grid gap-3 border-b border-line p-4 sm:grid-cols-2"><label><span className="mb-2 block text-sm font-semibold">Category</span><select value={category} disabled={Boolean(initial)} onChange={(event) => loadCategoryDefault(event.target.value as Category)} className="h-11 w-full rounded-md border border-line bg-white px-3 disabled:bg-slate-100">{categories.map((item) => <option key={item}>{item}</option>)}</select></label><label><span className="mb-2 block text-sm font-semibold">Template name</span><input value={name} onChange={(event) => setName(event.target.value)} className="h-11 w-full rounded-md border border-line px-3"/></label><label className="sm:col-span-2"><span className="mb-2 block text-sm font-semibold">Email subject</span><input value={subject} onChange={(event) => setSubject(event.target.value)} className="h-11 w-full rounded-md border border-line px-3"/></label><div className="flex gap-2 sm:col-span-2"><button onClick={() => { const source = defaults.find((template) => template.category === category); setSubject(source?.subject ?? category); setBlocks(cloneBlocks(source?.blocks ?? [])); }} className="rounded-md border border-line px-4 py-2 font-semibold">Reload original default</button><button onClick={() => setBlocks([])} className="px-4 py-2 font-semibold">Start from scratch</button></div></div>
