...
export function PropertyInfo({ value, onChange, editing }: { value: PropertyDetails; onChange: Dispatch<SetStateAction<PropertyDetails>>; editing: boolean; setToast: (message: string) => void }) {
  ...
  const locationQuery = value.latitude && value.longitude
    ? `${value.latitude},${value.longitude}`
    : [value.address, value.city, value.country].filter(Boolean).join(", ");
  const mapUrl = `https://www.google.com/maps?q=${encodeURIComponent(locationQuery)}&z=15&output=embed`;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <Input label="Language Code" value={value.languageCode} onChange={(v) => set("languageCode", v)} disabled={!editing} /><Input label="Timezone" value={value.timezone} onChange={(v) => set("timezone", v)} disabled={!editing} />
        <Input label="IBE Logo Width" value={value.ibeLogoWidth} onChange={(v) => set("ibeLogoWidth", v)} disabled={!editing} type="number" />
        <Input label="IBE Logo Height" value={value.ibeLogoHeight} onChange={(v) => set("ibeLogoHeight", v)} disabled={!editing} type="number" /><Input label="Support PIN (managed by security team)" value="" disabled />
      </div>
