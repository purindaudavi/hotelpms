...
export function BusinessBlockForm({ propertyId, businessDate, homeCurrency, ratePlans, roomTypes, block, onClose, onSave }: Props) {
  ...
  async function submit() {
    const validation = validateBlock(form, businessDate, Boolean(block), roomTypes);
...
function validateBlock(block: BusinessBlock, businessDate: string, editing: boolean, roomTypes: RoomTypeRecord[]) { if (!block.blockName.trim()) return "Block name is required."; if (!block.companyName.trim()) return "Company or Travel Agent is required."; if (block.checkOut <= block.checkIn) return "Check-out must be after check-in."; if (block.cutoffDate > block.checkIn) return "Cut-off date cannot be after check-in."; if (!editing && block.checkIn < businessDate) return "A new future block cannot start before the business date."; if (!block.allocations.length) return "At least one room allocation is required."; if (new Set(block.allocations.map((line) => line.roomTypeId)).size !== block.allocations.length) return "Duplicate room-type allocations are not allowed."; for (const line of block.allocations) { const type = roomTypes.find((item) => item.id === line.roomTypeId); if (!line.quantity || line.quantity < 1) return `Enter a quantity for ${line.roomTypeName}.`; if (line.quantity > (type?.rooms.length || 0)) return `${line.roomTypeName} has only ${type?.rooms.length || 0} configured rooms.`; if (line.negotiatedRate <= 0 && !line.isComplimentary) return `Enter a rate for ${line.roomTypeName}, or explicitly mark it complimentary.`; if (line.isComplimentary && !line.complimentaryReason?.trim()) return `Enter a complimentary reason for ${line.roomTypeName}.`; } if (block.depositPaid > block.depositRequired && block.depositRequired > 0) return "Deposit paid cannot exceed the required deposit."; return ""; }
