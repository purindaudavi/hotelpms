    ...
    <aside className="ml-auto flex h-full w-full max-w-[720px] flex-col bg-white shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5">
        ...
        {activeTab === "Overview" ? <><FinancialSummary currency={currency} total={booking.total} paid={paid} remaining={remaining} /><DetailSection title="Reservation Details" icon={<ClipboardList className="h-4 w-4" />}><DetailGrid items={[
          ["Reservation No", emptyValue(booking.resNo, "No reservation number")], ["Booking Reference", emptyValue(booking.bookingReference || booking.bookingRef, "No booking reference")], ["Booking Source", emptyValue(booking.bookingSource || booking.source, "No booking source")], ["Travel Agent", emptyValue(booking.travelAgentName, "No travel agent")], ["Agent Commission", booking.travelAgentId ? `${booking.travelAgentCommission ?? 0}%` : "Not applicable"], ["Created By", emptyValue(booking.createdBy, "Unknown creator")], ["Created Date / Time", formatTimestamp(booking.createdAt)], ["Check-in", formatDate(booking.checkIn)], ["Check-out", formatDate(booking.checkOut)], ["Status", booking.status], ["Room Use", booking.isDayRoom ? "Day room" : "Overnight"], ["Currency", currency], ["Rate Plan", emptyValue(booking.ratePlanName, "No rate plan")], ["Meal Plan", emptyValue(booking.mealPlan, "No meal plan")], ["Tour Number", emptyValue(booking.tourNumber, "No tour number")], ["Group Name", emptyValue(booking.groupName, "No group name")]
...
function RoomCard({ room, reservationCurrency, booking }: { room: ReservationRoom; reservationCurrency: string; booking: Reservation }) {
  ...
  const allocation = room.mealAllocationSnapshot;
  const mealNightly = allocation ? sumMealAmounts(allocation.adultAmounts) * room.adults + sumMealAmounts(allocation.childAmounts) * room.children : 0;
  return <div className={`rounded-md border p-4 text-sm ${room.isFoc ? "border-amber-300 bg-amber-50" : "border-line bg-slate-50"}`}><div className="flex flex-wrap justify-between gap-3"><div><b>{room.roomType}</b><p className="text-slate-500">Room {room.roomNumber || "Unassigned"}</p></div><b>{formatMoney(room.effectiveNightlyRate, currency)} / night</b></div><div className="mt-3 grid grid-cols-2 gap-2 text-slate-600 sm:grid-cols-4"><span>{room.occupancy}</span><span>{room.bedType}</span><span>{room.adults} adult(s)</span><span>{room.children} child(ren)</span></div><p className="mt-3 text-slate-600">{nights} chargeable night{nights === 1 ? "" : "s"} · {room.mealPlan || "No meal plan"} · {room.ratePlanName || "No rate plan"}</p>{allocation ? <p className="mt-2 rounded bg-blue-50 p-2 text-blue-900">Meal allocation snapshot: <b>{allocation.name}</b> · {formatMoney(mealNightly, allocation.currency)} per night for this room&apos;s guests. Included inside the room rate.</p> : null}{room.isFoc ? <p className="mt-2 text-amber-700">Complimentary (FOC): {room.focReason || "No FOC reason"}. Original rate {formatMoney(room.originalNightlyRate, currency)}.</p> : null}</div>;
}

function sumMealAmounts(amounts: { breakfast: number; lunch: number; dinner: number }) { return Number(amounts.breakfast || 0) + Number(amounts.lunch || 0) + Number(amounts.dinner || 0); }
