...
type Props = {
  ...
  onClose: () => void;
  onSave: (block: BusinessBlock) => Promise<string | void>;
};

export function BusinessBlockForm({ propertyId, businessDate, homeCurrency, ratePlans, roomTypes, block, onClose, onSave }: Props) {
  const [form, setForm] = useState<BusinessBlock>(() => block ? structuredClone(block) : newBusinessBlock(propertyId, businessDate, homeCurrency, ratePlans, roomTypes));
  ...
  function changeRatePlan(line: BusinessBlockAllocation, ratePlanId: string) {
    ...
    updateAllocation(line.id, { ratePlanId, ratePlanName: plan?.name || "", mealPlan: plan?.mealPlan || "Room Only", currency: plan?.currency || line.currency, negotiatedRate: plan?.roomTypeRates[line.roomTypeId] ?? line.negotiatedRate });
  }
  function addAllocation() {
    ...
    setForm((current) => ({ ...current, allocations: [...current.allocations, allocationDraft(propertyId, current.id, type.id, type.name, type.baseRate, homeCurrency, plan)] }));
  }
  ...
  async function submit() {
    const validation = validateBlock(form, businessDate, Boolean(block), roomTypes, ratePlans);
  ...
  return <div className="fixed inset-0 z-[60] bg-black/45"><aside className="ml-auto flex h-full w-full max-w-[1050px] flex-col bg-white shadow-2xl">
    ...
    <div className="flex-1 space-y-6 overflow-y-auto p-6">
      ...
      <FormSection title="Room allocations" action={<button type="button" onClick={addAllocation} className="inline-flex items-center gap-2 rounded-md bg-ink px-3 py-2 text-sm font-semibold text-white"><Plus className="h-4 w-4" />Add Room Type</button>}>
        <div className="overflow-x-auto"><table className="min-w-[1100px] text-left text-sm"><thead><tr className="border-b border-line text-slate-500">{["Room Type", "Quantity", "Rate Plan", "Meal Plan", "Currency", "Rate/Night", "Tax incl.", "FOC", "Reason", ""].map((heading) => <th key={heading} className="px-2 py-3">{heading}</th>)}</tr></thead><tbody>{form.allocations.map((line) => <tr key={line.id} className="border-b border-line">
          ...
          <td className="px-2 py-2"><select value={line.ratePlanId || ""} onChange={(event) => changeRatePlan(line, event.target.value)} className="h-10 w-40 rounded-md border border-line px-2"><option value="">Custom room-only rate</option>{ratePlans.filter((plan) => plan.active && (plan.mealPlan === "Room Only" || plan.mealAllocationId)).map((plan) => <option key={plan.id} value={plan.id}>{plan.name}</option>)}</select></td>
          <td className="px-2 py-2"><select disabled value={line.mealPlan} className="h-10 w-36 rounded-md border border-line bg-slate-100 px-2"><option>{line.mealPlan}</option></select></td>
    ...
    <footer className="flex justify-end gap-2 border-t border-line p-4"><button type="button" onClick={onClose} className="rounded-md border border-line px-5 py-2 font-semibold">Cancel</button><button type="button" disabled={saving} onClick={() => void submit()} className="rounded-md bg-ink px-5 py-2 font-semibold text-white disabled:opacity-60">{saving ? "Saving..." : "Save Business Block"}</button></footer>
  </aside></div>;
}

function newBusinessBlock(propertyId: string, businessDate: string, currency: string, ratePlans: RatePlan[], roomTypes: RoomTypeRecord[]): BusinessBlock {
  const id = createUuid(); const now = new Date().toISOString(); const type = roomTypes[0]; const plan = ratePlans[0];
  return { id, propertyId, blockNumber: `BB-${businessDate.replaceAll("-", "").slice(2)}-${id.slice(0, 4).toUpperCase()}`, blockName: "", companyName: "", contactName: "", contactEmail: "", contactPhone: "", checkIn: businessDate, checkOut: addDays(businessDate, 1), cutoffDate: businessDate, status: "Tentative", paymentMethod: "", billingParty: "Company", depositRequired: 0, depositPaid: 0, paymentDueDate: "", billingRemarks: "", cancellationPolicy: "", blockRemarks: "", internalRemarks: "", specialRequirements: "", allocations: [allocationDraft(propertyId, id, type.id, type.name, type.baseRate, currency, plan)], createdBy: currentSessionUser.name, createdAt: now, updatedAt: now };
}
function allocationDraft(propertyId: string, blockId: string, roomTypeId: string, roomTypeName: string, baseRate: number, currency: string, plan?: RatePlan): BusinessBlockAllocation { return { id: createUuid(), propertyId, businessBlockId: blockId, roomTypeId, roomTypeName, quantity: 1, ratePlanId: plan?.id || "", ratePlanName: plan?.name || "", mealPlan: plan?.mealPlan || "Room Only", currency: plan?.currency || currency, negotiatedRate: plan?.roomTypeRates[roomTypeId] ?? baseRate, taxInclusive: false, isComplimentary: false, complimentaryReason: "", releasedQuantity: 0 }; }
function validateBlock(block: BusinessBlock, businessDate: string, editing: boolean, roomTypes: RoomTypeRecord[], ratePlans: RatePlan[]) { if (!block.blockName.trim()) return "Block name is required."; if (!block.companyName.trim()) return "Company or Travel Agent is required."; if (block.checkOut <= block.checkIn) return "Check-out must be after check-in."; if (block.cutoffDate > block.checkIn) return "Cut-off date cannot be after check-in."; if (!editing && block.checkIn < businessDate) return "A new future block cannot start before the business date."; if (!block.allocations.length) return "At least one room allocation is required."; if (new Set(block.allocations.map((line) => line.roomTypeId)).size !== block.allocations.length) return "Duplicate room-type allocations are not allowed."; for (const line of block.allocations) { const type = roomTypes.find((item) => item.id === line.roomTypeId); const plan = ratePlans.find((item) => item.id === line.ratePlanId); if (!line.quantity || line.quantity < 1) return `Enter a quantity for ${line.roomTypeName}.`; if (line.quantity > (type?.rooms.length || 0)) return `${line.roomTypeName} has only ${type?.rooms.length || 0} configured rooms.`; if (line.mealPlan !== "Room Only" && (!plan || !plan.mealAllocationId)) return `Select a rate plan with a linked meal allocation for ${line.roomTypeName}.`; if (line.negotiatedRate <= 0 && !line.isComplimentary) return `Enter a rate for ${line.roomTypeName}, or explicitly mark it complimentary.`; if (line.isComplimentary && !line.complimentaryReason?.trim()) return `Enter a complimentary reason for ${line.roomTypeName}.`; } if (block.depositPaid > block.depositRequired && block.depositRequired > 0) return "Deposit paid cannot exceed the required deposit."; return ""; }
